...
import { User, Lock, ArrowLeft, Eye, EyeOff, GraduationCap, Hash, Users, UserPlus } from 'lucide-react'
import Link from 'next/link'
import { useRouter } from 'next/navigation'
import { authenticateMember, storeMemberAuth, checkMemberExists, type MemberLoginCredentials } from '@/lib/memberAuth'

export default function MemberLoginForm() {
  const [formData, setFormData] = useState<MemberLoginCredentials>({
    name: '',
    class: '',
    section: '',
    rollNo: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formData.name || !formData.class || !formData.section || !formData.rollNo || !formData.password) {
        throw new Error('Please fill in all fields')
      }

      // Try to authenticate
      const authData = await authenticateMember(formData)

      if (authData) {
        // Store authentication and redirect to member dashboard
        storeMemberAuth(authData)
        router.push('/member/dashboard')
      } else {
        // Check if member exists with these details (without password)
        const memberExists = await checkMemberExists(
          formData.name,
          formData.class,
          formData.section,
          formData.rollNo
        ...
        if (memberExists) {
          ...
          // Member doesn't exist, show registration option
          setShowRegistrationOption(true)
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        >
          ...
          {error && (
            ...
            >
              <p className={`font-calibri text-sm text-center ${
                showRegistrationOption ? 'text-blue-200' : 'text-red-200'
              }`}>
                {error}
              </p>
              {showRegistrationOption && (
                <motion.button
                  ...
                  className="w-full mt-3 bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-xl font-calibri font-semibold transition-colors duration-300"
                >
                  Create New Account
                </motion.button>
              )}
